                ...
                fs.createReadStream(zipPath)
                    ...
                    .on('entry', function (entry) {
                        ...
                        entry.on('end', function () {
                            let filename = fontFilename(folder, entry.path);
                            fs.writeFile(filename, jsonContent(data, filename), function (err) {
...
function jsonContent(data, filename) {
    let encodedContent = Buffer.concat(data).toString('base64'),
        json = '"css": "@font-face { font-family: \'placeholder\'; src: url(data:' + dataType(filename) + ';charset=utf-8;base64,' +
            encodedContent + '"';
...
function dataType(filename) {
    ...
    extension = filename.match(/\.(.+?)\.json$/)[1];
...
function fontFilename(folder, filename) {
    ...
    return folder + filename.replace(/v\d+-/, '') + '.json';
}
