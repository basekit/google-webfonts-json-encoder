...
function unzipFonts(options, callback) {
    ...
    fs.readdir(folder, function (err, files) {
        async.each(files, unzipFontFile.bind(null, folder), function (err) {
            callback(err);
        });
    });
}

function unzipFontFile(folder, filename, callback) {
    if (/\.zip$/.test(filename)) {
        ...
        fs.createReadStream(zipPath)
            ...
            .on('close', function () { closeZip(writer, zipPath, callback) });
    }
}

function closeZip (writer, zipPath, callback) {
    async.parallel([
